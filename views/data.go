...
func (d *Data) SetAlert(err error) {
	if pErr, ok := err.(PublicError); ok {
		d.Alert = &Alert{
			Level: AlertLvlError, Message: pErr.Public(),
		}
	} else {
		d.Alert = &Alert{
			Level:   AlertLvlError,
...
// RedirectAlert accepts all the normal params for http.Redirect and performs a redirect,
// but only after persisting the provided alert in the cookie so that it can be displayed
